...
class LogisticRegressionClassifier():
    ...
    def cost_grad(self, X, y, w):
        ...
        # average log likelihood (cost)
        log_sum = 0
        for i in range(X.shape[0]):
            log_sum += -np.log(logistic(y[i] * w.T @ X[i]))
        cost = log_sum / X.shape[0]

        # gradient
        log_sum = np.zeros(w.shape[0])
        for i in range(X.shape[0]):
            log_sum += -y[i] * X[i] * logistic(-y[i] * w.T @ X[i])
        grad = log_sum / X.shape[0]
